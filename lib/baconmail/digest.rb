  ...
  class Digest
    ...
    def self.generate_previews(new_messages, configs, email_address)
        ...
        new_messages.sort_by{|x| x[0]}.each do |m|
          ...
          AWS::S3::S3Object.store("#{prefix}_#{filename}.html", anonymize(m[2].to_s, configs), configs.bucket, :access => :public_read)
        end
    end

    def self.setup_redir
      content = "<html><head><meta charset='utf-8'><title>redirecting</title><script type='text/javascript'>var queryDict = {};location.search.substr(1).split('&').forEach(function(item) {queryDict[item.split('=')[0]] = item.split('=')[1]});window.location = unescape(queryDict.url);</script></head><body></body></html>"
      AWS::S3::S3Object.store("redir.html", content, configs.bucket, :access => :public_read)
    end

    def self.anonymize(message, configs)
      base_path                        = "http://#{configs.bucket}.s3.amazonaws.com/"
      doc                              = Nokogiri::HTML(message)
      doc.css("a").each{ |x| x['href'] = "#{base_path}?url=" + CGI.escape(x['href']) }
      return doc
    end
  end
end
